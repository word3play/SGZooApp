...
public class KKCustomEng implements com.konakart.appif.KKEngIf
{
     ...
     */
     ...
     private GetConfigurationNoCache _getConfigurationNoCache = null;
     ...
     private ReserveStock _reserveStock = null;
     private RemoveStockReservationsForBasketItems _removeStockReservationsForBasketItems = null;
     private RemoveStockReservationsForOrderProducts _removeStockReservationsForOrderProducts = null;
     private RemoveStockReservationsForIds _removeStockReservationsForIds = null;
     private CreateProductQuantityEncodedKeyFromEncodedProductId _createProductQuantityEncodedKeyFromEncodedProductId = null;
     private ExternalLogin _externalLogin = null;
    ...
    */
     ...
     * Depending on how much customer information you have, you may set booleans in the Customer
     * Registration object in order to instruct KonaKart to create default values. These booleans
     * are:
     * <ul>
     * <li>noGender - The gender attribute is populated with
     * com.konakart.bl.CustomerMgr.DEFAULT_STRING</li>
     * <li>noName - The first and last names are populated with
     * com.konakart.bl.CustomerMgr.DEFAULT_STRING</li>
     * <li>noBirthDate - The birth date is populated with com.konakart.bl.CustomerMgr.DEFAULT_DOB</li>
     * <li>noAddress - The address mandatory attributes are populated with
     * com.konakart.bl.CustomerMgr.DEFAULT_STRING.</li>
     * <li>noTelephone - The telephone attribute is populated with
     * com.konakart.bl.CustomerMgr.DEFAULT_STRING</li>
     * <li>noPassword - A random password is generated</li>
     * </ul>
     ...
     * negative. All registered customers have positive ids.
     * <p>
     * If in the options object,
     * <code>getStockReservationInfo is set to true, then stock reservation
     * information is returned. The extra attributes returned are
     * <ul>
     * <li><code>reservationId</code> - The identifier of the stock reservation.</li>
     * <li><code>qtyResrvdForResId</code> - The number of reserved items for the basket reservation
     * id. This is not always equal to the quantity of the basket item.</li>
     * <li><code>reservationExpiryDate</code> - The date time when the reservation expires.</li>
     * <li><code>reservationStartDate</code> - The date time when the reservation started.</li>
     * </ul>
     * <p>
    ...
    /**
     * Returns a Configuration object referenced by the key passed in as a parameter. The object is
     * read from the database and not the internal cache. Note that a KKException will be thrown if
     * the configuration key requested has not been marked to be returned by the APIs.
     * 
     * @param key
     *            The key of the configuration object
     * @return A Configuration object
     ...
     */
     public KKConfigurationIf getConfigurationNoCache(String key) throws KKException
     {
         try
         {
            if (_getConfigurationNoCache == null)
            {
                _getConfigurationNoCache = new GetConfigurationNoCache(kkEng);
            }

            return _getConfigurationNoCache.getConfigurationNoCache(key);
         } catch (Throwable e)
         {
            throw manageThrowable(e);
         }
     ...
     * dates.</li>
     * <li><code>escTool</code>: org.apache.velocity.tools.generic.EscapeTool object used to escape
     * characters.</li>
     * <li><code>listTool</code>: org.apache.velocity.tools.generic.ListTool object used to format
     * lists.</li>
     * <li><code>mathTool</code>: org.apache.velocity.tools.generic.MathTool object used to do sums.
     * </li>
     ...
     * than the size of the array passed in as a parameter.
     * <p>
     * If in the options object, <code>getStockReservationInfo</code> is set to true, then stock
     * reservation information is returned. As well as the <code>quantityInStock</code> and
     * <code>dateAvailable</code> attributes, the following attributes are also populated:
     * <ul>
     * <li><code>quantityReserved</code> - The total number of reserved similar items in the system.
     * </li>
     * <li><code>qtyResrvdForResId</code> - The number of reserved items for this basket reservation
     * id. This is not always equal to the quantity of the basket item.</li>
     * </ul>
     * If the basket stock reservation has expired, then <code>qtyResrvdForResId</code> will be set
     * to 0 and <code>reservationId</code> will be set to -1 and these changes will be reflected in
     ...
     * <ul>
     * <li>Define the number of seconds the reservation is active for. The default is set to -1
     * which means that the value defined in the global configuration variable is used. A batch job
     * should be scheduled to run at regular intervals to remove expired reservations.</li>
     * <li>Define whether you want to allow a partial reservation for a product if the number of
     * items available is less than the number required. The default behavior is to not allow
     * partial reservations.</li>
     * <li>Define a maximum number of reservations per customer for any one product. This is a
     * defensive measure to prevent a customer from abandoning multiple checkouts or a (single
     * checkout with a high quantity number) and so reserving products and preventing other
     * customers from buying them.</li>
     * </ul>
     * If a reservation already exists, it is deleted and a new one is inserted. The Basket items
     * returned, as well as having the reservation information populated, also have populated the
     * <code>quantityInStock</code> attribute. The reservation information is also persisted in the
     * database.
     * <p>
     * If any of the Basket items contain products or product variations no longer in the catalog
     * then they are not included in the return array.
     * 
     * @param sessionId
     *            The session id of the logged in user.
     * @param basketItems
     *            An array of basket items existing in the database.
     * @param options
     *            An object containing options for the method. It may be set to null.
     * @return Returns an array of basket items with the attributes <code>reservationId</code>,
     *         <code>qtyResrvdForResId</code> and <code>reservationExpiryDate</code> populated. The
     *         <code>reservationId</code> is set to -1 if the reservation could not be made because
     *         the stock was not available or for any other reason.
     * @throws KKException
     */
     public BasketIf[] reserveStock(String sessionId, BasketIf[] basketItems, StockReservationOptionsIf options) throws KKException
     {
         try
         {
            if (_reserveStock == null)
            {
                _reserveStock = new ReserveStock(kkEng);
            }

            return _reserveStock.reserveStock(sessionId, basketItems, options);
         } catch (Throwable e)
         {
            throw manageThrowable(e);
         }
    ...
    /**
     * This API call is used to remove stock reservations. The reservation ids are retrieved from
     * the array of basket items passed in as a parameter. The code checks that the reservations
     * belong to the customer referenced by the session id.
     * 
     * @param sessionId
     *            The session id of the logged in user
     * @param basketItems
     ...
     *            set to null.
     * @return Returns the number of reservations removed
     * @throws KKException
     */
     public int removeStockReservationsForBasketItems(String sessionId, BasketIf[] basketItems, StockReservationOptionsIf options) throws KKException
     {
         try
         {
            if (_removeStockReservationsForBasketItems == null)
            {
                _removeStockReservationsForBasketItems = new RemoveStockReservationsForBasketItems(kkEng);
            }

            return _removeStockReservationsForBasketItems.removeStockReservationsForBasketItems(sessionId, basketItems, options);
         } catch (Throwable e)
         {
            throw manageThrowable(e);
         }
    ...
    /**
     * This API call is used to remove stock reservations. The reservation ids are retrieved from
     * the array of order products passed in as a parameter. The code checks that the reservations
     * belong to the customer referenced by the session id.
     * 
     * @param sessionId
     *            The session id of the logged in user
     * @param orderProducts
     *            An array of OrderProduct items attached to an order.
     * @param options
     *            An object containing options for the method. It's not currently used and may be
     *            set to null.
     * @return Returns the number of reservations removed
     ...
     */
     public int removeStockReservationsForOrderProducts(String sessionId, OrderProductIf[] orderProducts, StockReservationOptionsIf options) throws KKException
     {
         try
         {
            if (_removeStockReservationsForOrderProducts == null)
            {
                _removeStockReservationsForOrderProducts = new RemoveStockReservationsForOrderProducts(kkEng);
            }

            return _removeStockReservationsForOrderProducts.removeStockReservationsForOrderProducts(sessionId, orderProducts, options);
         } catch (Throwable e)
         {
            throw manageThrowable(e);
         }
    ...
    /**
     * This API call is used to remove stock reservations. The reservation ids are retrieved from
     * the array of ids passed in as a parameter. The code checks that the reservations belong to
     * the customer referenced by the session id.
     * 
     * @param sessionId
     *            The session id of the logged in user
     * @param idArray
     *            An array of stock reservation ids
     * @param options
     *            An object containing options for the method. It's not currently used and may be
     *            set to null.
     * @return Returns the number of reservations removed
     ...
     */
     public int removeStockReservationsForIds(String sessionId, int[] idArray, StockReservationOptionsIf options) throws KKException
     {
         try
         {
            if (_removeStockReservationsForIds == null)
            {
                _removeStockReservationsForIds = new RemoveStockReservationsForIds(kkEng);
            }

            return _removeStockReservationsForIds.removeStockReservationsForIds(sessionId, idArray, options);
         } catch (Throwable e)
         {
            throw manageThrowable(e);
         }
     ...
     * <p>
     * The format of the encoded product id is 1{2}3{4}5 where the product id = 1, the first
     * optionId / valueId pair have ids of 2,3 and the 2nd optionId / valueId pair have ids of 4,5.
     * <p>
     * The format of the encoded key for the products_quantitytable is 3{4}4{6} which means
     * product_options of id==3 and product_options_values of id==4 followed by product_options of
     * id==4 and product_options_values of id==6. The order is very important since the product
     * option ids must be in ascending order.
     * 
     * @param encodedProductId
     * @return Returns a ProductQuantity object containing the product id and the encoded option
     *         values
     * @throws KKException
     */
     public ProductQuantityIf createProductQuantityEncodedKeyFromEncodedProductId( String encodedProductId) throws KKException
     {
         try
         {
            if (_createProductQuantityEncodedKeyFromEncodedProductId == null)
            {
                _createProductQuantityEncodedKeyFromEncodedProductId = new CreateProductQuantityEncodedKeyFromEncodedProductId(kkEng);
            }

            return _createProductQuantityEncodedKeyFromEncodedProductId.createProductQuantityEncodedKeyFromEncodedProductId(encodedProductId);
         } catch (Throwable e)
         {
            throw manageThrowable(e);
         }
     ...
     * 
     * @param loginInfo
     *            Contains the information required by the module to perform the login
     * @return Returns an ExternalLoginResult method containing a KonaKart sessionId if the login
     *         was successful
     * @throws KKException
     */
     public ExternalLoginResultIf externalLogin(ExternalLoginInputIf loginInfo) throws KKException
     {
         try
         {
            ...
            {
                _externalLogin = new ExternalLogin(kkEng);
            }

            return _externalLogin.externalLogin(loginInfo);
         } catch (Throwable e)
         {
            throw manageThrowable(e);
         }
